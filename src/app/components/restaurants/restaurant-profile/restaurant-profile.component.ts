...
export class RestaurantProfileComponent implements OnInit, OnChanges {
  ...
  Explanations = {
    ChineseExplanations: {
      ...
      "PreventOnlineTipIfCash": "",
      "ServiceFeeTaxableIfNonCashTx": "对qMenu服务费/佣金随订单小计一同计税。在此餐馆中，部分或所有服务费用是由顾客承担的，当顾客使用信用卡支付时，需要将因这些服务费而产生的税费一同支付给餐馆。"
    },
    EnglishExplanations: {
      ...
      "PreventOnlineTipIfCash": "",
      "ServiceFeeTaxableIfNonCashTx": "Add tax on top of the qMenu service fee/commission as well as the subtotal. For this restaurant, to the extent all or some of the qMenu service fee burden is placed on the customer, when the customer pays by credit card, the restaurant will incur tax on the service fee and needs to be compensated for it by the customer."
    }
  }
  ...
  requiredFieldsMissing() {
    // additional required fields can be added as desired. form editor will not allow saving until all required fields are filled.
    const REQUIRED_FIELDS = ["taxRate", "preferredLanguage"];
    const pickupEnabled = ((((this.restaurant.serviceSettings || []).find(s => s.name === "Pickup")) || {}).paymentMethods || []).length > 0;
    const deliveryEnabled = ((((this.restaurant.serviceSettings || []).find(s => s.name === "Delivery")) || {}).paymentMethods || []).length > 0;
