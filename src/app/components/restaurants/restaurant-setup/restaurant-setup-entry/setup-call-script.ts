...
const deliverySectionCallScript = {
  ChineseCallScript: {
    ...
    offer_inquiry: `
    请问，你们自己提供外送服务吗？
    <br/>- (不提供？没问题！qMenu可以为你们提供！我们只向客户收取1.98 + 基于距离的外送费。我现在可以为你开启。)[除非餐馆特意要求不开启，否则打开 Postmates服务]
    <br/>- (好的！我们会继续为您配置外送设置。)[继续询问与自助外送相关的问题]
    `,
  ...
  EnglishCallScript: {
    ...
    <br/>- (No? Ok, no problem! qMenu can actually facilitate delivery for you! We only charge the customer $1.98 + distance-based delivery fees. I can turn that on for you now.) [wait for the restaurant to say "please don't", otherwise, turn on Postmates]
    <br/>- (Ok, sounds good! We'll go ahead and set up your delivery settings for you.) [proceed to ask self-delivery-related questions]
    `,
...
const pickupPaymentCallScript = {
  ChineseCallScript: {
    ...
    rt_collect_inquiry: '[如果餐馆自己收款]: 好的，您是希望等客户取餐时刷卡，还是我们当客户下单时将信用卡信息发送给您来输入？(或如果您有自己的 Stripe 帐户，我们可以链接到该帐户)'
  },
  EnglishCallScript: {
    ...
    rt_collect_inquiry: '[If RT collects themselves]: Alright, so the customer will swipe in person when they get their food, or should we send you the CC number for key-in when they place the order? (Or do you have your own Stripe account that we should link to?)'
  }
}
...
const invoicingSectionCallScript = {
  ChineseCallScript: {
    ...
    receiving_inquiry: '收到来自 qMenu 的付款，您希望如何收到这些付款？ 获得直接存款，还是邮寄支票？[做出相应的选择]',
    send_inquiry: '而如果向 qMenu 付款，您更喜欢哪种方式？ 直接取款，通过信用卡付款，还是寄支票给qMenu呢？[做出相应的选择]',
  ...
  EnglishCallScript: {
    ...
    receiving_inquiry: 'If receiving a payout from qMenu, how would you like to receive those payouts? Get a direct deposit, or just a check in the mail? [make selections accordingly]',
    send_inquiry: 'And if sending a payment to qMenu, which method would you prefer? Direct withdrawal, paying via credit card, or sending us a check? [make selections accordingly]',
