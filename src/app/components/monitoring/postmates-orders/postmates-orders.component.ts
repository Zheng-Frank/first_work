...
@Component({
  ...
  styleUrls: ['./postmates-orders.component.css']
})
export class PostmatesOrdersComponent implements OnInit {
  ...
  */
  async searchOrderByDate() {
    if (this.dateType != filterTypes.Custom) {
      let query = {};
      ...
      for (let i = 0; i < tempCustomerIds.length; i++) {
        ...
        const subPreviousOrders = await this._api.getBatch(environment.qmenuApiUrl + "generic", {
          resource: "order",
          query: {
            'customerObj._id': { $in: tempCustomerIds[i] }
          },
          projection: {
            _id: 1,
            customer: 1
  ...
  async handleOnAdjustInvoice(order) {
    this.orderForModal = order;
